...
func test_sample_empty_curve_returns_zero() -> void:
	assert_eq(
		DayNightEnvScript._sample([], 12.0),
		0.0,
...
func test_sample_before_first_keyframe() -> void:
	...
	assert_eq(
		DayNightEnvScript._sample(curve, 2.0),
		10.0,
...
func test_sample_at_first_keyframe() -> void:
	var curve := [[5.0, 10.0], [10.0, 20.0]]
	assert_eq(
		DayNightEnvScript._sample(curve, 5.0),
		10.0,
...
func test_sample_at_last_keyframe() -> void:
	var curve := [[5.0, 10.0], [10.0, 20.0]]
	assert_eq(
		DayNightEnvScript._sample(curve, 10.0),
		20.0,
...
func test_sample_after_last_keyframe() -> void:
	var curve := [[5.0, 10.0], [10.0, 20.0]]
	assert_eq(
		DayNightEnvScript._sample(curve, 15.0),
		20.0,
...
func test_sample_quarter_interpolation() -> void:
	var curve := [[0.0, 0.0], [10.0, 100.0]]
	assert_almost_eq(
		DayNightEnvScript._sample(curve, 2.5),
		25.0,
		0.01,
...
func test_sample_multi_segment() -> void:
	...
	assert_almost_eq(
		DayNightEnvScript._sample(curve, 7.5),
		75.0,
		0.01,
...
func test_sun_pitch_midnight_is_negative() -> void:
	var pitch := DayNightEnvScript._sample(DayNightEnvScript.SUN_PITCH, 0.0)
	assert_lt(pitch, 0.0, "Sun should be below horizon at midnight")


func test_sun_pitch_noon_is_positive() -> void:
	var pitch := DayNightEnvScript._sample(DayNightEnvScript.SUN_PITCH, 12.0)
	assert_gt(pitch, 0.0, "Sun should be above horizon at noon")


func test_sun_pitch_noon_is_highest() -> void:
	var noon := DayNightEnvScript._sample(DayNightEnvScript.SUN_PITCH, 12.0)
	var morning := DayNightEnvScript._sample(DayNightEnvScript.SUN_PITCH, 7.0)
	var evening := DayNightEnvScript._sample(DayNightEnvScript.SUN_PITCH, 17.0)
...
func test_sun_energy_zero_at_midnight() -> void:
	var energy := DayNightEnvScript._sample(DayNightEnvScript.SUN_ENERGY, 0.0)
	assert_eq(energy, 0.0, "Sun energy should be 0 at midnight")


func test_sun_energy_one_at_midday() -> void:
	var energy := DayNightEnvScript._sample(DayNightEnvScript.SUN_ENERGY, 12.0)
	assert_eq(energy, 1.0, "Sun energy should be 1.0 at midday")


func test_sun_energy_zero_at_night() -> void:
	var energy := DayNightEnvScript._sample(DayNightEnvScript.SUN_ENERGY, 22.0)
...
func test_ambient_energy_low_at_night() -> void:
	var energy := DayNightEnvScript._sample(DayNightEnvScript.AMBIENT_ENERGY, 0.0)
	assert_eq(energy, 0.05, "Ambient energy should be minimal at night")


func test_ambient_energy_higher_at_day() -> void:
	var energy := DayNightEnvScript._sample(DayNightEnvScript.AMBIENT_ENERGY, 12.0)
	assert_gt(
		energy,
		0.1,
...
func test_sky_top_blue_higher_during_day() -> void:
	var night_b := DayNightEnvScript._sample(DayNightEnvScript.SKY_TOP_B, 0.0)
	var day_b := DayNightEnvScript._sample(DayNightEnvScript.SKY_TOP_B, 12.0)
	assert_gt(
		day_b,
		night_b,
...
func test_sky_horizon_red_peaks_at_sunset() -> void:
	var sunset := DayNightEnvScript._sample(DayNightEnvScript.SKY_HOR_R, 18.0)
	var midday := DayNightEnvScript._sample(DayNightEnvScript.SKY_HOR_R, 12.0)
	assert_gt(
		sunset,
		midday,
...
func test_sun_pitch_at_noon_is_positive() -> void:
	...
	var noon_pitch: float = DayNightEnvScript._sample(DayNightEnvScript.SUN_PITCH, 12.0)
...
func test_ambient_energy_spans_24() -> void:
	_assert_curve_spans_24(
		DayNightEnvScript.AMBIENT_ENERGY,
		"AMBIENT_ENERGY",
	)
...
func test_weather_fog_has_three_entries() -> void:
	assert_eq(
		DayNightEnvScript.WEATHER_FOG.size(),
		3,
...
func test_weather_cloud_a_has_three_entries() -> void:
	assert_eq(
		DayNightEnvScript.WEATHER_CLOUD_A.size(),
		3,
...
func test_weather_cloud_g_has_three_entries() -> void:
	assert_eq(
		DayNightEnvScript.WEATHER_CLOUD_G.size(),
		3,
...
func test_star_sphere_radius_constant() -> void:
	assert_almost_eq(
		DayNightEnvScript.STAR_SPHERE_R,
		4800.0,
		0.1,
