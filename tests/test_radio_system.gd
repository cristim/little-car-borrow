...
func test_genre_tempo_min_less_than_max() -> void:
	var genres := [
		RadioScript.GENRE_POP,
		RadioScript.GENRE_ROCK,
		RadioScript.GENRE_JAZZ,
		RadioScript.GENRE_ELECTRONIC,
		RadioScript.GENRE_CLASSICAL,
	]
	for g in genres:
...
func test_five_drum_patterns_defined() -> void:
	assert_eq(
		RadioScript.DRUM_PATTERNS.size(),
		5,
...
func test_progressions_have_4_chords() -> void:
	for i in RadioScript.PROGRESSIONS.size():
		assert_eq(
			RadioScript.PROGRESSIONS[i].size(),
			4,
...
func test_progression_chords_have_3_notes() -> void:
	for i in RadioScript.PROGRESSIONS.size():
		for j in RadioScript.PROGRESSIONS[i].size():
			assert_eq(
				RadioScript.PROGRESSIONS[i][j].size(),
				3,
...
func test_police_lines_wanted_not_empty() -> void:
	assert_gt(
		RadioScript.POLICE_LINES_WANTED.size(),
		0,
...
func test_police_lines_calm_not_empty() -> void:
	assert_gt(
		RadioScript.POLICE_LINES_CALM.size(),
		0,
...
func test_electronic_uses_arp_mode() -> void:
	assert_eq(
		RadioScript.GENRE_ELECTRONIC.melody_mode,
		"arp",
...
func test_rock_uses_power_chord_mode() -> void:
	assert_eq(
		RadioScript.GENRE_ROCK.melody_mode,
		"power_chord",
...
func test_only_rock_has_distortion() -> void:
	assert_gt(
		RadioScript.GENRE_ROCK.dist_drive,
		0.0,
