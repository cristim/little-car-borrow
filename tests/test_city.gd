...
func test_chunk_load_radius() -> void:
	assert_eq(
		CityScript.CHUNK_LOAD_RADIUS, 2.5,
...
func test_chunk_unload_radius() -> void:
	assert_eq(
		CityScript.CHUNK_UNLOAD_RADIUS, 3.5,
...
func test_update_interval() -> void:
	assert_eq(
		CityScript.UPDATE_INTERVAL, 0.5,
...
func test_max_cascading_repairs() -> void:
	assert_eq(
		CityScript.MAX_CASCADING_REPAIRS, 12,
...
func test_dir_offsets_north() -> void:
	assert_eq(
		CityScript.DIR_OFFSETS[0], Vector2i(0, -1),
...
func test_dir_offsets_east() -> void:
	assert_eq(
		CityScript.DIR_OFFSETS[1], Vector2i(1, 0),
...
func test_dir_offsets_south() -> void:
	assert_eq(
		CityScript.DIR_OFFSETS[2], Vector2i(0, 1),
...
func test_dir_offsets_west() -> void:
	assert_eq(
		CityScript.DIR_OFFSETS[3], Vector2i(-1, 0),
...
func test_opposite_directions_cancel() -> void:
	for dir: int in CityScript.DIR_OFFSETS:
		...
		var sum: Vector2i = (
			CityScript.DIR_OFFSETS[dir] + CityScript.DIR_OFFSETS[opposite]
		)
		assert_eq(
			sum, Vector2i.ZERO,
...
func test_add_sidewalk_collision_z_axis() -> void:
	var body := StaticBody3D.new()
	add_child_autofree(body)
	CityScript.add_sidewalk_collision(
		body, Vector3.ZERO, 2.5, 10.0, 0.1, 1.0, "z",
	)
	var col: CollisionShape3D = body.get_child(0) as CollisionShape3D
	assert_not_null(col)
...
func test_add_sidewalk_collision_x_axis() -> void:
	...
	add_child_autofree(body)
	CityScript.add_sidewalk_collision(
		body, Vector3.ZERO, 2.5, 10.0, 0.1, 1.0, "x",
	)
	var col: CollisionShape3D = body.get_child(0) as CollisionShape3D
	assert_not_null(col)
	var shape: ConvexPolygonShape3D = col.shape as ConvexPolygonShape3D
...
func test_st_add_face_with_door_creates_three_strips() -> void:
	...
	CityScript.st_add_face_with_door(
		...
		10.0, 4.0,          # face_width, face_height
		Vector3(0, 0, -1),  # normal
		Vector3(1, 0, 0),   # right
		2.0, 2.5,           # door_width, door_height
	)
	var mesh: ArrayMesh = st.commit()
	var arrays := mesh.surface_get_arrays(0)
	var verts: PackedVector3Array = arrays[Mesh.ARRAY_VERTEX]
