...
func _process(delta : float) -> void:
	if wheel_node:
		...
		if not is_zero_approx(beam_axle):
			var wheel_lookat_vector: Vector3 = (opposite_wheel.transform * opposite_wheel.wheel_node.position) - (transform * wheel_node.position)
...
func process_forces(opposite_compression : float, braking : bool, delta : float) -> float:
	...
	if is_colliding() and last_collider:
		...
		var contact: Vector3 = last_collision_point - vehicle.global_position
		if spring_force > 0.0:
