...
func _physics_process(_delta: float) -> void:
	...
	if not _was_on_floor and on_floor:
		...
		if fall_dist > FALL_DAMAGE_MIN_HEIGHT:
			var dmg: float = (fall_dist - FALL_DAMAGE_MIN_HEIGHT) * FALL_DAMAGE_PER_METER
...
func _on_interaction_area_entered(area: Area3D) -> void:
	if area.is_in_group("vehicle_interaction"):
		...
		if sm.current_state and sm.current_state.name.to_lower() in foot_states:
			var is_boat: bool = nearest_vehicle.get_node_or_null("BoatController") != null
			var is_heli: bool = nearest_vehicle.get_node_or_null("HelicopterController") != null
			var prompt := "Hold F to board" if (is_boat or is_heli) else "Hold F to steal"
