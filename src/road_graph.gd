...
func find_path(
	from_world: Vector3, to_world: Vector3, grid: RefCounted
) -> Array[Vector3]:
	...
	var start_pos: Vector3 = _get_intersection_pos(
		from_ns, from_ew, from_world, grid
	)
	var goal_pos: Vector3 = _get_intersection_pos(
		to_ns, to_ew, to_world, grid
	)
	...
	var start_key: Vector3i = _make_key(
		from_ns, from_ew, start_pos, grid_span
	)
	var goal_key: Vector3i = _make_key(
		to_ns, to_ew, goal_pos, grid_span
	)
	...
	while open_list.size() > 0 and iterations < MAX_ITERATIONS:
		...
		if current_key == goal_key:
			return _reconstruct_path(
				came_from, positions, current_key, start_key
			)
		...
		var neighbors: Array = _get_neighbors(
			ns_idx, ew_idx, current_pos, grid
		)
		...
		for neighbor in neighbors:
			...
			var n_key: Vector3i = _make_key(
				n_ns, n_ew, n_pos, grid_span
			)
			...
			_insert_sorted(
				open_list, [n_f, tentative_g, n_key, n_pos]
			)

	return []


func _get_intersection_pos(
	ns_idx: int, ew_idx: int, ref: Vector3, grid: RefCounted
) -> Vector3:
...
func _make_key(
	ns_idx: int, ew_idx: int, world_pos: Vector3, grid_span: float
) -> Vector3i:
...
func _get_neighbors(
	ns_idx: int, ew_idx: int, current_pos: Vector3,
	grid: RefCounted,
) -> Array:
	...
	var n_pos: Vector3 = _get_intersection_pos(
		ns_idx, n_ew, n_ref, grid
	)
	...
	var s_pos: Vector3 = _get_intersection_pos(
		ns_idx, s_ew, s_ref, grid
	)
	...
	var e_pos: Vector3 = _get_intersection_pos(
		e_ns, ew_idx, e_ref, grid
	)
	...
	var w_pos: Vector3 = _get_intersection_pos(
		w_ns, ew_idx, w_ref, grid
	)
...
func _reconstruct_path(
	came_from: Dictionary, positions: Dictionary,
	current_key: Vector3i, start_key: Vector3i,
) -> Array[Vector3]:
